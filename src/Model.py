...
class Model: 
	...
	def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False):
		...
		self.train_writer = tf.summary.FileWriter( './logs/1/train ', self.sess.graph)

	def builloss(self):
		loss = tf.reduce_mean(tf.nn.ctc_loss(labels=self.gtTexts, inputs=self.ctcIn3dTBC, sequence_length=self.seqLen, ctc_merge_repeated=True))
	...
	def trainBatch(self, batch):
		...
		tf.summary.scalar('loss', lossVal)
		
		merge += tf.summary.merge_all()
		self.batchesTrained += 1
		self.train_writer.add_summary(merge, self.batchesTrained )
		return lossVal
